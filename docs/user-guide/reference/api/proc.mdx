---
title: "Proc<S,M> API Reference"
description: "Lightweight virtual-thread process with mailbox - Java 26 equivalent of Erlang/OTP processes"
sidebar_position: 1
---
...
#### `spawn<S,M>(S initial, BiFunction<S, M, S> handler)`
...
#### `Proc<S,M>(S initial, BiFunction<S, M, S> handler)`
...
- [Supervisor API](./supervisor.mdx) — Hierarchical process supervision with restart strategies
- [ProcRef API](./procref.mdx) — Stable handle that survives supervisor restarts
- [ProcLink API](./proclink.mdx) — Bilateral crash propagation between processes
- [ProcMonitor API](./procmonitor.mdx) — Unilateral DOWN notifications
- [StateMachine API](./statemachine.mdx) — Complex workflows with sealed transitions
- [EventManager API](./eventmanager.mdx) — Typed event broadcasting
- [JOTP Architecture](/docs/architecture) — Overall system design
