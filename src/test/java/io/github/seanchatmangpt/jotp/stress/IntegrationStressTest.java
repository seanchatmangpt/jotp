...
class IntegrationStressTest extends StressTestBase {
   ...
   *
   * <p>Setup: Supervisor manages 10 worker processes. EventManager broadcasts work events. Workers
   * process events at constant 100 msg/sec per worker (1000 total). Measure end-to-end latency.
   *
  ...
  void testSystemDurabilityWithCrashes() {
    Supervisor supervisor =
        new Supervisor(Strategy.ONE_FOR_ONE, 50, Duration.ofSeconds(30));
    ...
    try {
      ...
      LoadProfile profile =
          new LoadProfile.ConstantLoad(100L, Duration.ofSeconds(30));
      MetricsCollector metrics =
          runStressTest(
  ...
  void testEventManagerMixedHandlers() {
    ...
    try {
      ...
      // Load: 500 events/sec
      LoadProfile profile =
          new LoadProfile.ConstantLoad(500L, Duration.ofSeconds(5));
      MetricsCollector metrics =
          runStressTest(
              "EventManager Mixed Handlers",
              profile,
              () -> {
                String largeEvent = "event-" + System.nanoTime() + "-large-payload-data";
