 ...
 *
 * <p>Performs multi-level benchmarking across load spectrum: 1K → 100K msg/sec, collecting resource
 * metrics (CPU, memory, GC, thread allocation) at each level. Uses PerformanceModel to derive:
 *
 * <ul>
...
class EnterpriseCapacityBenchmark extends StressTestBase {
     ...
     *
     * <p>Collects resource metrics at each level: - Throughput - Latency distribution (p50, p99,
     * p999, max) - Heap usage (current, peak) - GC pause time and frequency - CPU utilization -
     * Virtual thread count - Error rate
     */
    @Test
    ...
    void benchmarkProcCapacity() {
        ...
        try {
            for (long load : loadLevels) {
                ...
                // Resource metrics
                double cpuUtilization = (cpuTimeMs > 0) ? (100.0 * cpuTimeMs) / wallclockTimeMs : 0;
    ...
    void benchmarkScalingEfficiency() {
        ...
        for (int i = 0; i < workerCounts.length; i++) {
            ...
            try {
                LoadProfile profile = new LoadProfile.ConstantLoad(10_000L, Duration.ofSeconds(5));
                MetricsCollector metrics =
                        runStressTest(
                ...
                System.out.printf("%d worker(s): %.0f msg/sec\n", workerCount, throughputs[i]);
        ...
        assertTrue(
                scaling4x > 2.0, "4 workers should yield >2x throughput, got " + scaling4x + "x");
    }
