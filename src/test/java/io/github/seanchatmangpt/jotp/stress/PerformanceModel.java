...
public final class PerformanceModel {
  ...
  public double memoryGrowthRate() {
    ...
    long totalLoadDelta =
        results.get(results.size() - 1).loadLevel - results.get(0).loadLevel;
  ...
  public String virtualThreadEfficiencyAnalysis() {
    ...
    double avgVtPerOp = results.stream()
        .mapToDouble(r -> r.virtualThreadsAllocated / (double) Math.max(r.actualThroughput, 1))
        .average()
        .orElse(0);

    double avgMemPerOp = results.stream()
        .mapToDouble(BenchmarkResult::memoryPerOpBytes)
        .average()
        .orElse(0);
    ...
    double platform_overhead = (avgVtPerOp / 4) * platformThreadCostMb; // assume 4:1 consolidation
  ...
  public CapacityPlan planCapacity(SlaTarget sla, double instanceCostAnnual) {
    ...
    long instancesNeeded = (sla.tps + slaBreakpoint.actualThroughput - 1) / slaBreakpoint.actualThroughput;
    ...
    if (slaBreakpoint.gcPauseImpactPercent() > 1.0) {
      constraints.add("GC pauses consuming " + (int) slaBreakpoint.gcPauseImpactPercent() + "% of runtime");
    }
    if (slaBreakpoint.p999LatencyMs > sla.p999LatencyMs) {
      constraints.add("p999 latency (" + (int) slaBreakpoint.p999LatencyMs + "ms) exceeds SLA (" + (int) sla.p999LatencyMs + "ms)");
    }
    if (slaBreakpoint.errorRate > sla.maxErrorRate) {
      constraints.add("Error rate (" + (int) slaBreakpoint.errorRate + "%) exceeds SLA (" + (int) sla.maxErrorRate + "%)");
    }

    String strategy = instancesNeeded <= 3 ? "Vertical scaling (add heap/CPU per instance)"
        : instancesNeeded <= 10 ? "Horizontal scaling (load balancer + N instances)"
  ...
  public String gcImpactAnalysis() {
    ...
    double avgP99Latency = results.stream().mapToDouble(r -> r.p99LatencyMs).average().orElse(0);

    double gcLatencyCorrelation = (avgGcPauseMs > 0) ? avgP99Latency / avgGcPauseMs : 0;

    String verdict = gcLatencyCorrelation > 2
    ...
    return String.format(
        ...
        verdict,
        gcLatencyCorrelation > 2 ? "Use ZGC/Shenandoah for sub-ms pauses" : "Current GC tuning acceptable");
  }
  ...
  public String costAnalysis(double cpuCostPerHourPerVcpu, double memCostPerGbHour) {
    ...
    double cpuCostAnnual = cpuCostPerHourPerVcpu * (peak.cpuUtilizationPercent / 100) * 24 * 365;
