 ...
 *
 * <p>Compares competing architectural approaches across multiple dimensions: - Throughput vs
 * latency (Pareto frontier) - Resource consumption (CPU, memory, threads) - Operational complexity
 * (monitoring, tuning, scaling) - Cost efficiency (CapEx, OpEx, cost-per-operation) -
...
public final class ArchitecturalTradeoffAnalysis {
     ...
     *
     * <p>Examples: "Virtual Threads + LinkedTransferQueue", "Platform Threads +
     * ArrayBlockingQueue", "Actor Model (Akka)", etc.
     */
    public static class Architecture implements Comparable<Architecture> {
        ...
        /**
         * Latency predictability: lower stddev = more predictable (we score as 1 - coefficient of
         * variation)
         */
     ...
     *
     * <p>Inputs: - Infrastructure annual cost per instance - Operational cost (SRE time per year) -
    ...
    public Map<Architecture, Double> calculateTCO(
        ...
        for (Architecture arch : paretoFrontier()) {
            ...
            long instancesNeeded =
                    Math.max(1, 100_000 / (long) Math.max(arch.throughputOpsPerSec, 1));
            ...
            double opsCost =
                    instancesNeeded
                            * operationalCostPerInstanceYear
                            * (1 + arch.complexityScore / 10.0);

            // Availability cost (availability = 1 - errorRate, rough estimate)
            double availability = 0.9995; // assume 99.95% baseline
            double downtime = 365 * 24 * (1 - availability); // hours/year
            double opportunityCost = downtime * opportunityCostPerHourUnavailability;
     ...
     *
     * <p>Weights: - 40% throughput (max absolute throughput) - 30% latency (minimize p99) - 20%
     * cost (minimize TCO) - 10% operational simplicity
     */
    public Map<Architecture, Double> scoreArchitectures() {
        ...
        double maxThroughput =
                architectures.stream().mapToDouble(a -> a.throughputOpsPerSec).max().orElse(1);
        double minLatency = architectures.stream().mapToDouble(a -> a.latencyP99Ms).min().orElse(1);
        double maxCost =
                architectures.stream().mapToDouble(a -> a.costPerBillionOps).max().orElse(1);
     ...
     *
     * <p>Decision logic: - If latency <1ms required: pick lowest latency architecture - If
     * throughput >500K required: pick highest throughput architecture - If cost <$1per billion ops
     * required: pick lowest cost - Otherwise: pick Pareto best overall score
     */
