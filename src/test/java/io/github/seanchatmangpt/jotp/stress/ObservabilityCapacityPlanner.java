...
/**
 * Capacity planning tests for observability infrastructure.
 *
...
class ObservabilityCapacityPlanner {
     ...
     *
     * <p>SLA: <10MB per 1000 events
     */
    @Test
    @DisplayName("Memory overhead: <10MB per 1000 events")
    void memoryOverhead() throws Exception {
        int eventCount = 1_000;
        FrameworkEventBus eventBus = FrameworkEventBus.create();
        FrameworkMetrics metrics = FrameworkMetrics.create(eventBus);
        ...
        for (int i = 0; i < eventCount; i++) {
            eventBus.publishFrameworkEvent("test_event_" + i, ConcurrentHashMap.newKeySet());
            metrics.recordProcessMessage(
                    new ProcessMetrics.ProcessIdentifier(
                            "test_proc", Thread.currentThread().toString()),
                    ProcessMetrics.MessageType.SENT,
                    1L);
        }
        ...
        long memoryOverhead = memoryUsed / (eventCount / 1000); // per 1000 events
        ...
        System.out.printf("Overhead per 1K events: %,d bytes%n", memoryOverhead);
        System.out.printf("SLA target: %,d bytes%n", MEMORY_PER_1K_EVENTS);
        System.out.println();

        assertTrue(
                memoryOverhead < MEMORY_PER_1K_EVENTS,
                String.format(
                        "Memory overhead %,d bytes exceeds SLA %,d bytes",
                        memoryOverhead, MEMORY_PER_1K_EVENTS));
    }

    /**
     * Test: Hot path contamination validation
     *
     * <p>SLA: Zero hot path contamination (<1% overhead)
     */
    @Test
    @DisplayName("Hot path contamination: <1% overhead")
    void hotPathContamination() throws Exception {
        ...
        System.out.printf("SLA target: <1.00%%%n");
        System.out.println();

        assertTrue(
                overhead < 1.0,
                String.format("Hot path overhead %.2f%% exceeds SLA <1.00%%", overhead));
    }
                            ...
                            try {
                                ...
                                while (publishedCount.get() < targetThroughput) {
                                    ...
                                    try {
                                        ...
                                        eventBus.publishFrameworkEvent(
                                                "capacity_test_event_" + processId,
    ...
    /** Baseline hot path without observability. */
    private long runHotPathBaseline(int iterations) {
        long startTime = System.nanoTime();

        for (int i = 0; i < iterations; i++) {
            // Simulate hot path work: simple computation
            int result = i * 2 + 1;
    ...
    /** Hot path with observability enabled. */
    private long runHotPathWithObservability(int iterations) {
        FrameworkEventBus eventBus = FrameworkEventBus.create();
        FrameworkMetrics metrics = FrameworkMetrics.create(eventBus);
        ...
        for (int i = 0; i < iterations; i++) {
            ...
            // Minimal observability (realistic hot path)
            eventBus.publishFrameworkEvent("hot_path_event", ConcurrentHashMap.newKeySet());
        }
