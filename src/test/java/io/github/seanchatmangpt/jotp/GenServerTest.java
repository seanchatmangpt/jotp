 ...
 * state deterministically. We use {@link org.awaitility.Awaitility} for async assertions and
 * {@link CountDownLatch} to synchronize callback firing.
 *
 * @see GenServer
...
class GenServerTest {
  ...
  void testCastFireAndForget() throws InterruptedException {
    var handler =
        new GenServer.Handler<Integer, CounterMsg>() {
          @Override
          public GenServer.CallResult<Integer> handleCall(CounterMsg request, Integer state) {
  ...
  void testInfoMessages() throws Exception {
    ...
    await()
        ...
        .untilAsserted(() -> assertThat(infoCount.get()).isEqualTo(3));
