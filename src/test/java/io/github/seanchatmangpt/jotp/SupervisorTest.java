 ...
 * </ul>
 *
 * <p><strong>Max Restarts Throttling:</strong> If a single child crashes more than {@code maxRestarts}
 * times within the {@code window} time period, the supervisor gives up and terminates itself
 * (including all children). This prevents infinite restart loops.
 *
...
class SupervisorTest {
  ...
  void testOneForOneSingleChildCrash() throws Exception {
    ...
    var ref1 =
        supervisor.supervise(
            ...
            0,
            (state, msg) -> {
              if (msg instanceof TestMsg.Crash) throw new RuntimeException("child-1 crash");
    ...
    await().atMost(AWAIT_TIMEOUT).until(() -> !supervisor.isRunning() || ref1.proc().lastError != null);
  ...
  void testRestForOneCrashRestartsFromPosition() throws Exception {
    ...
    var ref2 =
        supervisor.supervise(
            ...
            100,
            (state, msg) -> {
              if (msg instanceof TestMsg.Crash) throw new RuntimeException("child-2 crash");
              if (msg instanceof TestMsg.Increment) return state + 1;
              return state;
            });
  ...
  void testConcurrentChildCrashes() throws Exception {
    var supervisor = new Supervisor(Supervisor.Strategy.ONE_FOR_ONE, 10, Duration.ofSeconds(60));
    var childCount = 5;
    ...
    for (int i = 0; i < childCount; i++) {
      ...
      refs[i] =
          supervisor.supervise(
              ...
              i * 100,
              (state, msg) -> {
                if (msg instanceof TestMsg.Crash) throw new RuntimeException("child-" + idx);
                if (msg instanceof TestMsg.Increment) return state + 1;
                return state;
              });
    }
