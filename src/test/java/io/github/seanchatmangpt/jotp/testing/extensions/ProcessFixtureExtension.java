 ...
 *
 * <ul>
 *   <li>Discover sealed Message type hierarchies via {@code Class.permittedSubclasses()}
 *   <li>Extract record components: {@code Class.getRecordComponents()}
 *   <li>Resolve generic types: {@code Type.getActualTypeArguments()}
 *   <li>Create process instances via constructor introspection
 * </ul>
 *
 * <p>Features:
 *
 * <ul>
 *   <li>Auto-cleanup after test (deregistration, termination)
 *   <li>Supervision strategy configuration (ONE_FOR_ONE, ONE_FOR_ALL, REST_FOR_ONE)
 *   <li>Optional ProcessRegistry registration
 *   <li>Message capture during fixture lifecycle
 * </ul>
 */
public class ProcessFixtureExtension
        implements TestInstancePostProcessor, ExtensionContext.Store.CloseableResource {
    ...
    private final List<AutoCloseable> cleanupActions =
    ...
    private void createFixtures(
            Object testInstance, ProcessFixture annotation, ExtensionContext context)
            throws Exception {
        ...
        for (int i = 0; i < instances; i++) {
            ...
            if (annotation.registerInRegistry()) {
                var registryName =
                        annotation.registryName().isEmpty()
    ...
    /**
     * Create process instance via reflection on sealed type constructors. Inspects class hierarchy
     * using Java 26 reflection API.
     */
    ...
    /**
     * Resolve generic type arguments via Java 26 reflection. Example: {@code Proc<State, Message>}
     * → extracts State and Message
     */
