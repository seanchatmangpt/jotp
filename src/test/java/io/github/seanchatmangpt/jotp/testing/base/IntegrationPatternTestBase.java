 ...
 *
 * <ul>
 *   <li>Router → Aggregator → Filter
 *   <li>Splitter → ScatterGather → Resequencer
 *   <li>ProcessManager → DeadLetterChannel → Supervisor
 * </ul>
 *
 * <p>Features:
 *
 * <ul>
 *   <li>Manages process chains and interconnections
 *   <li>Validates data flow through pattern boundaries
 *   <li>Tracks messages across all processes
 *   <li>Validates end-to-end causality
 * </ul>
 *
 * <p>Usage:
 *
...
public abstract class IntegrationPatternTestBase {
    ...
    protected void validateNoMessageLoss(int expectedCount) {
        ...
        if (chains.size() != expectedCount) {
            throw new AssertionError(
                    "Message loss detected: expected "
                            + expectedCount
                            + " correlation chains, got "
                            + chains.size());
        }
    }
    ...
    public String toString() {
        return "IntegrationPatternTest[patterns="
                + patternInstances.size()
                + ", chainLength="
                + processChain.size()
                + "]";
    }
