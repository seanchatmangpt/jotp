...
import io.github.seanchatmangpt.jotp.dogfood.otp.JvmCrashRecoveryPatterns.ChargeReceipt;
...
class JvmCrashRecoveryPatternsTest implements WithAssertions {
  ...
  class IdempotentCharge {
    ...
    @Test
    @DisplayName("sending same idempotency key twice returns the same transaction ID (deduplication)")
  ...
  class CheckpointReplay {
    ...
    void checkpointHandlerAdvancesCounters() throws InterruptedException {
      ...
      proc.ask(new JvmCrashRecoveryPatterns.ProcessorMessage.ProcessItem("i1", 1), timeout).join();
      proc.ask(new JvmCrashRecoveryPatterns.ProcessorMessage.ProcessItem("i2", 1), timeout).join();
      var afterCheckpoint =
    ...
    @Test
    @DisplayName("un-checkpointed items are captured between processedCount and checkpointCount")
    void unCheckpointedItemsAreDetectable() throws InterruptedException {
      ...
      proc.ask(new JvmCrashRecoveryPatterns.ProcessorMessage.ProcessItem("second", 1), timeout)
          .join();
      var state =
          proc.ask(new JvmCrashRecoveryPatterns.ProcessorMessage.ProcessItem("third", 1), timeout)
              .join();
