 ...
 *
 * <p>Patterns 34–39 of the EIP catalog implemented using Java 25+ virtual threads, sealed
 * interfaces, records, and structured concurrency primitives from {@code org.acme.reactive}:
 *
 * <ol start="34">
 *   <li>Dynamic Router — routing table updated at runtime without restart
 *   <li>Content Enricher — augments message with external data before forwarding
 *   <li>Wire Tap — non-intrusive message observation via virtual thread fork
 *   <li>Polling Consumer — consumer controls retrieval rate (pull-based endpoint)
 *   <li>Durable Subscriber — buffers messages across pause/resume cycles
 *   <li>Resequencer — restores message order from an out-of-sequence stream
 * </ol>
 *
 * <p>Together with {@link ReactiveMessagingFoundationPatternsTest} (10 patterns), {@link
 * ReactiveMessagingRoutingPatternsTest} (9 patterns), and {@link
 * ReactiveMessagingEndpointPatternsTest} (14 patterns), this file completes coverage of all 39
 * canonical Enterprise Integration Patterns.
 */
@Timeout(60)
...
class ReactiveMessagingSystemPatternsTest implements WithAssertions {
    ...
    class DynamicRouterPattern {
        ...
        void routesStatically() {
            ...
            await().atMost(Duration.ofSeconds(2))
                    .until(() -> premiumReceived.size() == 2 && standardReceived.size() == 1);
        ...
        void prependedRouteWins() {
            ...
            await().atMost(Duration.ofSeconds(2))
                    .until(() -> normalReceived.size() == 2 && overrideReceived.size() == 1);
        ...
        void removeRouteRedirectsToDefault() {
            ...
            await().atMost(Duration.ofSeconds(1))
                    .untilAsserted(() -> assertThat(dead.size()).isEqualTo(1));
    ...
    class ContentEnricherPattern {
        ...
        void simpleMappingEnricher() {
            ...
            var enricher =
                    ContentEnricher.of((String s) -> s.toUpperCase() + "_ENRICHED", downstream);
    ...
    class WireTapPattern {
        ...
        void tapReceivesCopy() throws InterruptedException {
            ...
            await().atMost(Duration.ofSeconds(2))
                    .until(() -> primaryReceived.size() == 3 && tapReceived.size() == 3);
    ...
    class PollingConsumerPattern {
        ...
        void pollsAndProcesses() throws Exception {
            ...
            try (var consumer =
                    new PollingConsumer<Integer>(processed::add, Duration.ofMillis(10))) {
                consumer.send(1);
                consumer.send(2);
                consumer.send(3);
        ...
        void usesExternalQueue() throws Exception {
            ...
            try (var consumer =
                    new PollingConsumer<>(externalQueue, processed::add, Duration.ofMillis(10))) {
        ...
        void crashingHandlerDoesNotStopPolling() throws Exception {
            ...
            try (var consumer =
                ...
                await().atMost(Duration.ofSeconds(2))
                        .until(() -> processed.get() + failed.get() == 10);
    ...
    class ResequencerPattern {
        ...
        void reordersOutOfSequenceMessages() throws Exception {
            ...
            try (var resequencer =
                    new Resequencer<SeqMsg, Long>(SeqMsg::seq, n -> n + 1, downstream)) {
                ...
                assertThat(ordered.stream().map(SeqMsg::seq)).containsExactly(0L, 1L, 2L, 3L, 4L);
        ...
        void holdsMesagesUntilGapFilled() throws Exception {
            ...
            try (var resequencer =
                    new Resequencer<Integer, Integer>(n -> n, n -> n + 1, downstream)) {
    ...
    class SystemPatternsIntegration {
        ...
        void composedSystemPatternPipeline() throws Exception {
            ...
            try (var inbound = new DurableSubscriber<Order>(enricher::send)) {
                ...
                await().atMost(Duration.ofSeconds(3))
                        .until(() -> premiumDelivered.size() == 2 && standardDelivered.size() == 2);
        ...
        void pollingConsumerFeedsResequencer() throws Exception {
            ...
            try (var resequencer = new Resequencer<Long, Long>(n -> n, n -> n + 1, downstream);
                    var poller =
                            new PollingConsumer<Long>(resequencer::send, Duration.ofMillis(5))) {
