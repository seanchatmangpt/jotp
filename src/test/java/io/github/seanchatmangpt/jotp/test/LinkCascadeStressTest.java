    ...
    void chainCascade_500deep_allDieWithin5s() throws Exception {
        ...
        System.out.printf("[chain-cascade] depth=%d elapsed=%d ms%n", depth, elapsedMs);
                ...
                java.util.Map.of(
                        ...
                        "Per-hop latency", String.format("%.2f ms", elapsedMs / (double) depth),
                        "Threshold", "<10ms per hop",
                        "Result", elapsedMs < 5000 ? "PASS" : "FAIL"));
    }
    ...
    void deathStar_1000workers_hubCrashKillsAll() throws Exception {
        ...
        assertThat(workers).noneMatch(w -> w.thread().isAlive());
                ...
                java.util.Map.of(
                        ...
                        "Propagation time",
                        String.format("%d ms", elapsedMs),
                        "Interrupts fired",
                        "1000 concurrent",
                        "Dead workers verified",
                        "100%",
                        "Result",
                        elapsedMs < 5000 ? "PASS" : "FAIL"));
    }
