 ...
 *
 * <p>Tests Vaughn Vernon's Enterprise Integration Patterns for fault tolerance: Supervision Storm,
 * Dead Letter Channel + Control Bus + Supervisor Integration.
 *
 * <p>In race telemetry, resilience is critical - we cannot lose data during a race. The system must
 * recover from failures automatically without operator intervention.
 *
...
class AtlasResiliencePatternsIT implements WithAssertions {
    ...
    class SupervisionStormPattern {
        ...
        void parameterCrashRestartWithStableRef() throws Exception {
            ...
            for (int i = 0; i < 50; i++) {
                paramProcessor.tell(
                        new Sample(
                                new ParameterId("P" + i), new Timestamp(i), (short) i, new Good()));
            }
            ...
            paramProcessor.tell(
                    new Sample(
                            new ParameterId("CRASH"),
                            new Timestamp(-1),
                            (short) -999,
                            new InvalidData("crash")));

            await().atMost(Duration.ofSeconds(3))
                    .until(() -> crashes.get() >= 1 && sup.isRunning());
        ...
        void multipleChildCrashesWithOneForAll() throws Exception {
            var sup =
                    new Supervisor(
                            "one-for-all-sv", Strategy.ONE_FOR_ALL, 10, Duration.ofMinutes(5));
            ...
            processor1.tell(
                    new Sample(new ParameterId("P1"), new Timestamp(0), (short) 1, new Good()));
            processor2.tell(
                    new Sample(new ParameterId("P2"), new Timestamp(1), (short) 2, new Good()));
            processor3.tell(
                    new Sample(new ParameterId("P3"), new Timestamp(2), (short) 3, new Good()));

            await().atMost(Duration.ofSeconds(2))
                    .until(
                            () ->
                                    processor1Count.get() >= 1
                                            && processor2Count.get() >= 1
                                            && processor3Count.get() >= 1);
            ...
            processor1.tell(
                    new Sample(
                            new ParameterId("P1_POST"),
                            new Timestamp(100),
                            (short) 100,
                            new Good()));
            processor2.tell(
                    new Sample(
                            new ParameterId("P2_POST"),
                            new Timestamp(101),
                            (short) 101,
                            new Good()));
            processor3.tell(
                    new Sample(
                            new ParameterId("P3_POST"),
                            new Timestamp(102),
                            (short) 102,
                            new Good()));
        ...
        void supervisionWithProcessMonitor() throws Exception {
            ...
            var processor =
                    sup.supervise(
                            ...
                            (Integer s, Sample msg) -> {
                                if (msg.rawValue() == -1)
                                    throw new RuntimeException("Monitored crash");
                                return s + 1;
                            });
            ...
            Proc<Integer, Sample> underlyingProc =
                    (Proc<Integer, Sample>)
                            processor.getClass().getDeclaredField("delegate").get(processor);
            ...
            var sup2 =
                    new Supervisor("monitor-sv2", Strategy.ONE_FOR_ONE, 10, Duration.ofMinutes(5));
    ...
    class DeadLetterControlBusSupervisorIntegration {
        ...
        void deadLetterPlusControlBusPlusSupervisor() throws Exception {
            ...
            mainProcessor.tell(
                    new Sample(
                            new ParameterId("INVALID"),
                            new Timestamp(100),
                            (short) -1,
                            new InvalidData("Negative")));
            mainProcessor.tell(
                    new Sample(
                            new ParameterId("INVALID"),
                            new Timestamp(101),
                            (short) 2000,
                            new OutOfRange(0, 1000)));
            ...
            await().atMost(Duration.ofSeconds(1))
                    .until(() -> currentMode.get().equals("MAINTENANCE"));

            // Samples during maintenance go to dead letter
            mainProcessor.tell(
                    new Sample(
                            new ParameterId("MAINT"), new Timestamp(200), (short) 100, new Good()));

            await().atMost(Duration.ofSeconds(2)).until(() -> rejected.get() == 3);

            // Verify dead letter count using type-safe QueryState
            var dlState = deadLetterProc.ask(new QueryState.DeadLetters()).get(2, TimeUnit.SECONDS);
    ...
    class FullRaceTelemetryFlow {
        ...
        void fullRaceTelemetryFlow() throws Exception {
            var sup =
                    new Supervisor(
                            "race-telemetry-sv", Strategy.ONE_FOR_ONE, 100, Duration.ofMinutes(5));
            ...
            for (int lap = 1; lap <= 5; lap++) {
                ...
                // Lap marker
                sessionManager.tell(
                        new LapEventMsg(sessionId, new LapNumber(lap), new Timestamp(lap * 10000)));
            }
            ...
            await().atMost(Duration.ofSeconds(5))
                    .until(
                            () ->
                                    observedSamples.get() >= 100
                                            && observedLaps.get() == 5
                                            && observedEvents.get() >= 4);

            // Verify session state using type-safe QueryState
            var finalSession = sessionManager.ask(new QueryState.Full()).get(5, TimeUnit.SECONDS);
            ...
            var deadLetters =
                    deadLetterProc.ask(new QueryState.DeadLetters()).get(2, TimeUnit.SECONDS);
