 ...
 *
 * <p>Tests Vaughn Vernon's Enterprise Integration Patterns applied to all three McLaren Atlas API
 * surfaces:
 *
 * <ul>
 *   <li>SQLRaceAPI — Session lifecycle, parameters, samples, laps, statistics
 *   <li>FileSessionAPI — File save, load, streaming operations
 *   <li>DisplayAPI — Display updates, plugin lifecycle, tool windows
 * </ul>
 *
...
class AtlasAllAPIsMessagePatternsIT implements WithAssertions {
    ...
    class SQLRaceAPIPatterns {
        ...
        void sessionWriteSampleAsEventMessage() throws Exception {
            ...
            record SampleStore(List<Sample> samples) {
                static SampleStore initial() {
                    return new SampleStore(new ArrayList<>());
                }
            }
            ...
            for (int i = 0; i < 100; i++) {
                sampleBus.notify(
                        new Sample(paramId, new Timestamp(i), (short) (1000 + i), new Good()));
            }
        ...
        void sessionGetParametersAsRequestReply() throws Exception {
            var params =
                    List.of(
                            ...
                            new ParameterSpec(
                                    new ParameterId("BRAKE_TEMP_FL"),
                                    "Brake Temp FL",
                                    0,
                                    1200,
                                    "C"),
                            new ParameterSpec(
                                    new ParameterId("TIRE_PRESSURE_FL"),
                                    "Tire Pressure FL",
                                    0,
                                    30,
                                    "bar"));
            ...
            record SessionState(
                    List<ParameterSpec> parameters, Map<ParameterId, List<Sample>> data) {
        ...
        void sessionCreateLapAsCorrelationId() throws Exception {
            ...
            var lapManager =
                    new Proc<>(
                            LapData.initial(sessionId),
                            (LapData s, Object msg) -> {
                                if (msg instanceof LapEventMsg lap
                                        && lap.sessionId().id().equals(s.sessionId().id())) {
            ...
            for (int i = 1; i <= 5; i++) {
                lapManager.tell(
                        new LapEventMsg(sessionId, new LapNumber(i), new Timestamp(i * 10000L)));
            }
        ...
        void sessionGetStatisticsAsDocumentMessage() throws Exception {
            record SessionStats(
                    int totalSamples, int lapCount, Map<ParameterId, Integer> samplesPerParam) {
            ...
            var stats =
                    new Proc<>(
                            SessionStats.initial(),
                            (SessionStats s, Object msg) -> {
                                if (msg instanceof Sample sample) {
            ...
            for (int i = 0; i < 50; i++) {
                ...
                stats.tell(
                        new Sample(
                                brakeParam, new Timestamp(i + 100), (short) (i * 2), new Good()));
            }
        ...
        void fullSQLRaceSessionWithMessagePatterns() throws Exception {
            var sup =
                    new Supervisor("sqlrace-sv", Strategy.ONE_FOR_ONE, 100, Duration.ofMinutes(5));
            var sessionId = SessionId.generate();
            ...
            var session =
                    sup.supervise(
                            "session",
                            SQLRaceSession.initial(sessionId),
                            (SQLRaceSession s, Object msg) -> {
                                return switch (msg) {
                                    case SessionCmd.Open cmd ->
                                            new SQLRaceSession(
                                                    cmd.id(),
                                                    new SessionState.Initialized(),
                                                    s.laps(),
                                                    s.data());
                                    case SessionCmd.Configure _ ->
                                            new SQLRaceSession(
                                                    s.id(),
                                                    new SessionState.Configured(),
                                                    s.laps(),
                                                    s.data());
                                    case SessionCmd.GoLive _ ->
                                            new SQLRaceSession(
                                                    s.id(),
                                                    new SessionState.GoLive(),
                                                    s.laps(),
                                                    s.data());
                                    case SessionCmd.Record _ ->
                                            new SQLRaceSession(
                                                    s.id(),
                                                    new SessionState.Recording(),
                                                    s.laps(),
                                                    s.data());
                                    case SessionCmd.Save _ ->
                                            new SQLRaceSession(
                                                    s.id(),
                                                    new SessionState.Saving(),
                                                    s.laps(),
                                                    s.data());
                                    case SessionCmd.Close _ ->
                                            new SQLRaceSession(
                                                    s.id(),
                                    ...
                                    case Sample sample -> {
                                        if (s.state() instanceof SessionState.Recording) {
                                            ...
                                            newData.computeIfAbsent(
                                                            ...
                                                            k -> new ArrayList<>())
                                                    .add(sample.rawValue());
                                            yield new SQLRaceSession(
                                                    s.id(), s.state(), s.laps(), newData);
                                        }
                                        yield s;
                                    }
            ...
            for (int i = 0; i < 100; i++) {
                telemetryBus.notify(
                        new Sample(
                                rpmParam, new Timestamp(i), (short) (5000 + i * 10), new Good()));
            }
            ...
            await().atMost(Duration.ofSeconds(3))
                    .until(
                            () -> {
                                var s =
                                        session.ask(new QueryState.Full())
                                                .get(100, TimeUnit.MILLISECONDS);
    ...
    class FileSessionAPIPatterns {
        ...
        void fileSessionSaveAsClaimCheck() throws Exception {
            ...
            // Claim check record
            record ClaimCheck(
                    SessionId sessionId, String location, int sampleCount, long checksum) {}

            // File store state
            record FileStoreState(
                    Map<SessionId, ClaimCheck> claims, Map<SessionId, List<Sample>> cache) {
        ...
        void fileSessionLoadAsContentFilter() throws Exception {
            ...
            for (int i = 0; i < 50; i++) {
                ...
                samples.add(
                        new Sample(brakeParam, new Timestamp(i + 50), (short) (i * 2), new Good()));
            }
        ...
        void fileSessionStreamAsMessageSequence() throws Exception {
            ...
            record StreamState(
                    int expectedSeq, List<Sample> assembled, Map<Integer, StreamBatch> pending) {
                                ...
                                if (msg instanceof StreamBatch batch) {
                                    ...
                                    if (batch.seqNum() == s.expectedSeq()) {
                                        ...
                                        return new StreamState(
                                                s.expectedSeq(), s.assembled(), newPending);
                                    }
                                }
                                return s;
                            });
    ...
    class DisplayAPIPatterns {
        ...
        void displayUpdateAsEventDrivenConsumer() throws Exception {
            ...
            // Display state
            record DisplayState(int rpm, Map<Integer, Double> brakeTemps, List<Double> lapTimes) {
                            ...
                            (DisplayState s, Object msg) -> {
                                if (msg instanceof DisplayEvent.RpmUpdate rpm) {
                                    ...
                                    return new DisplayState(
                                            rpm.value(), s.brakeTemps(), s.lapTimes());
        ...
        void pluginInitializationAsServiceActivator() throws Exception {
            ...
            record PluginState(
                    String name, boolean initialized, boolean running, List<String> log) {
        ...
        void toolWindowAsMessageBus() throws Exception {
            ...
            // Tool window manager
            record ToolWindowState(Map<String, String> windows, String focusedWindow) {
    ...
    class CrossAPIOrchestration {
        ...
        void fullPipelineSQLRaceToFileSessionToDisplay() throws Exception {
            ...
            record SQLRaceState(SessionId id, SessionState state, int sampleCount) {
                static SQLRaceState initial() {
                    return new SQLRaceState(null, new SessionState.Initialized(), 0);
                }
            }
            ...
            var session =
                    sup.supervise(
                            "session",
                            SQLRaceState.initial(),
                            (SQLRaceState s, Object msg) -> {
                                if (msg instanceof Sample sample) {
            ...
            record FileSessionState(int samplesSinceSave, List<Sample> pending) {
                static FileSessionState initial() {
            ...
            var fileStore =
                    sup.supervise(
                            "filestore",
                            FileSessionState.initial(),
                            (FileSessionState s, Object msg) -> {
                                if (msg instanceof Sample sample) {
            ...
            var display =
                    sup.supervise(
                            ...
                            (DisplayState s, Object msg) -> {
                                if (msg instanceof Sample sample
                                        && sample.parameterId().id().equals("ENGINE_RPM")) {
            ...
            for (int i = 0; i < 75; i++) {
                session.tell(
                        new Sample(
                                rpmParam, new Timestamp(i), (short) (5000 + i * 100), new Good()));
            }
            ...
            await().atMost(Duration.ofSeconds(3))
                    .until(
                            () -> {
                                var dispState =
                                        display.ask(new QueryState.Full())
                                                .get(100, TimeUnit.MILLISECONDS);
        ...
        void crossAPIEventCorrelation() throws Exception {
            ...
            interface CorrelatedEvent {
                ...
                record SampleReceived(SessionId sessionId, Sample sample)
                        implements CorrelatedEvent {}

                record FileSaved(SessionId sessionId, String path) implements CorrelatedEvent {}

                record DisplayUpdated(SessionId sessionId, String component)
                        implements CorrelatedEvent {}
            }
                                ...
                                if (msg instanceof CorrelatedEvent event) {
                                    ...
                                    return switch (event) {
                                        case CorrelatedEvent.SampleReceived sr -> {
                                            ...
                                            yield new CorrelationState(
                                                    newSamples,
                                                    s.filesPerSession(),
                                                    s.displaysPerSession());
                                        }
            ...
            correlator.tell(
                    new CorrelatedEvent.SampleReceived(
                            sessionId,
                            new Sample(
                                    new ParameterId("RPM"),
                                    new Timestamp(0),
