...
/**
 * Vaughn Vernon's Reactive Messaging Endpoint & Transformation Patterns with JOTP. Tests: Channel
 * Adapter, Messaging Bridge, Message Bus, Pipes and Filters, Message Dispatcher, Event-Driven
 * Consumer, Competing Consumers, Selective Consumer, Idempotent Receiver, Service Activator,
...
class ReactiveMessagingEndpointPatternsTest implements WithAssertions {
    ...
    class ChannelAdapterPattern {
        @Test
        void bridgeExternalToMailbox() throws Exception {
    ...
    class MessagingBridgePattern {
        @Test
        void bridgeTwoChannels() throws Exception {
            var ch2Received = new AtomicInteger(0);
            var channel2 =
                    new Proc<>(
                            0,
                            (Integer s, String msg) -> {
                                ch2Received.incrementAndGet();
                                return s + 1;
                            });
            var bridge =
                    new Proc<>(
                            channel2,
                            (Proc<Integer, String> ch2, String msg) -> {
    ...
    class MessageBusPattern {
        @Test
        void eventManagerAsMessageBus() throws Exception {
            var orderCount = new AtomicInteger(0);
            var paymentCount = new AtomicInteger(0);
            var bus = EventManager.<Map.Entry<String, Object>>start();
            bus.addHandler(
                    e -> {
                        if (e.getKey().startsWith("order.")) orderCount.incrementAndGet();
                    });
            bus.addHandler(
                    e -> {
                        if (e.getKey().startsWith("payment.")) paymentCount.incrementAndGet();
                    });
    ...
    class PipesAndFiltersPattern {
        @Test
        void processingChain() throws Exception {
            ...
            var filter1 =
                    new Proc<>(
                            sink,
                            (Proc<Integer, String> next, String msg) -> {
                                next.tell("processed:" + msg);
                                return next;
                            });
            filter1.tell("hello");
            filter1.tell("world");
    ...
    class MessageDispatcherPattern {
        @Test
        void loadBalanceAcrossWorkers() throws Exception {
            int workerCount = 5;
    ...
    class EventDrivenConsumerPattern {
        @Test
        void reactiveHandling() throws Exception {
            var received = new AtomicInteger(0);
            var consumer =
                    new Proc<>(
                            new ArrayList<String>(),
                            (List<String> log, String msg) -> {
    ...
    class CompetingConsumersPattern {
        @Test
        void competingConsumers() throws Exception {
    ...
    class SelectiveConsumerPattern {
        @Test
        void selectiveConsumption() throws Exception {
            ...
            await().atMost(Duration.ofSeconds(2))
                    .until(() -> accepted.get() == 3 && rejected.get() == 2);
            consumer.stop();
        }
    }
    ...
    class IdempotentReceiverPattern {
        ...
        void deduplication() throws Exception {
            ...
            await().atMost(Duration.ofSeconds(2))
                    .until(() -> uniqueCount.get() == 3 && duplicateCount.get() == 2);
            receiver.stop();
        }
    }
    ...
    class ServiceActivatorPattern {
        @Test
        void serviceActivated() throws Exception {
            var invocations = new AtomicInteger(0);
            var activator =
                    new Proc<>(
                            invocations,
                            (AtomicInteger count, String msg) -> {
    ...
    class ClaimCheckPattern {
        @Test
        void storePayloadSendReference() throws Exception {
    ...
    class Integration {
        @Test
        void completeMessageFlow() throws Exception {
            var sup =
                    new Supervisor("endpoint-sv", Strategy.ONE_FOR_ONE, 100, Duration.ofMinutes(5));
        ...
        @Test
        void faultTolerantEndpoint() throws Exception {
