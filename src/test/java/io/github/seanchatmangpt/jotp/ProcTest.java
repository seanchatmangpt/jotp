 ...
 *
 * <p>Tests cover message delivery (tell/ask), mailbox ordering (FIFO), crash callbacks,
 * termination callbacks, exit signal trapping, process suspension/resumption, and concurrent
 * sender scenarios.
 *
 ...
 *   <li><strong>Crash Callbacks:</strong> Fired only on abnormal termination (unhandled
 *       exception).
...
class ProcTest {
  ...
  void testMailboxOrdering() throws Exception {
    ...
    await()
        ...
        .until(() -> received.size() == messageCount);
  ...
  void testCrashCallbackFiredOnException() throws Exception {
    ...
    var handler =
        (Integer state, TestMsg msg) -> {
          if (msg instanceof TestMsg.Crash) throw new RuntimeException("intentional crash");
          return state;
        };
   ...
   */
  private static <S, M> Proc<S, M> createTestProc(S initialState, java.util.function.BiFunction<S, M, S> handler) {
   ...
   * @param timeout wait timeout
   */
  private static void assertCallbackFired(CountDownLatch latch, Duration timeout) throws Exception {
