...
class PatternsIntegrationTest {
  ...
  void testEventSourcingAuditTrailWithSaga() {
    var coordinator =
        new EcommerceOrderService.OrderSagaCoordinator(
            paymentService, inventoryService, shippingService, auditLog, Duration.ofSeconds(5));
    ...
    for (int i = 0; i < 10; i++) {
      ...
      if (result.isFailure()) {
        ...
        if (!error.compensations().isEmpty()) {
          ...
          var lastCompensation = error.compensations().get(error.compensations().size() - 1);
  ...
  void testCircuitBreakerStateTransitions() {
    ...
    var breaker = CircuitBreaker.create("test-cb", 2, Duration.ofSeconds(5), Duration.ofMillis(500));
    ...
    assertThat(state).isIn(CircuitBreaker.State.CLOSED, CircuitBreaker.State.OPEN, CircuitBreaker.State.HALF_OPEN);
  }
