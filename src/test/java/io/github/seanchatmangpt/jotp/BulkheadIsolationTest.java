 ...
 *
 * <p>Tests cover bulkhead creation, message routing, load balancing, status transitions
 * (Active → Degraded → Failed), rejection handling, and concurrent failure scenarios.
 *
  ...
  void testRejectionOnFailed() {
    ...
    await()
        ...
        .untilAsserted(
            () -> {
              var status = bulkhead.status();
              assertThat(status).isInstanceOf(BulkheadIsolation.BulkheadStatus.Failed.class);
            });
