    ...
    public static void exampleStateMachineCreate() {
        ...
        StateMachine<LockState, LockEvent, LockData> sm =
                StateMachine.of(
                        ...
                        new LockData("", "1234"),
                        (state, event, data) -> {
                            if (event instanceof StateMachine.SMEvent.User<?> userEvent
                                    && userEvent.event() instanceof LockEvent.PushButton(var b)) {
                                var entered = data.entered() + b;
                                return entered.equals(data.code())
                            ...
                            if (event instanceof StateMachine.SMEvent.User<?> userEvent
                                    && userEvent.event() instanceof LockEvent.Lock) {
