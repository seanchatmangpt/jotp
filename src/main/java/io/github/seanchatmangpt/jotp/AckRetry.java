 ...
 * solution is for the sender to monitor the target, detect the {@code DOWN} signal, and resend to
 * a restarted process. {@code AckRetry} brings this pattern to Java 26 virtual-thread processes:
 *
 * <ul>
...
public final class AckRetry {
   ...
   *
   * <p>Uses {@link CrashRecovery#retry} internally: each attempt runs in an isolated virtual thread
   * and calls {@link Proc#ask(Object, Duration)}. The first successful reply is returned
   * immediately. If all {@code maxAttempts} are exhausted, the last exception is returned as a
   * {@link Result.Failure}.
   *
   * <p>Retries are immediate (no sleep between attempts). For backoff, use
   * {@link #sendWithBackoff}.
   *
  ...
  /**
   * Executes a single ask attempt in an isolated virtual thread, wrapping the result in a
   * {@link Result}.
   */
