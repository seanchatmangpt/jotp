 ...
 *
 * <p>Martin Fowler: "The basic idea behind the circuit breaker is very simple. You wrap a protected
 * function call in a circuit breaker object, which monitors for failures. Once the failures reach a
 * certain threshold, the circuit breaker trips, and all further calls to the circuit breaker return
 * with an error."
 *
 * <p>Joe Armstrong: "In Erlang, we let it crash. But we also need to protect downstream services. A
 * circuit breaker is like a supervisor for remote calls."
 *
 * <p>Features:
 *
 * <ul>
 *   <li><b>Three states</b> — CLOSED, OPEN, HALF_OPEN with automatic transitions
 *   <li><b>Failure threshold</b> — Configurable number of failures before tripping
 *   <li><b>Timeout handling</b> — Per-operation timeout with automatic failure recording
 *   <li><b>Reset timeout</b> — Time to wait before attempting HALF_OPEN
 *   <li><b>Result-based API</b> — Returns {@link Result} with typed errors
 * </ul>
 *
 * <p><b>Usage:</b>
 *
...
public final class CircuitBreaker {
    ...
    public record Config(
            String name, int failureThreshold, Duration timeout, Duration resetTimeout) {}
    ...
    public static final class Builder {
        ...
        public CircuitBreaker build() {
            return new CircuitBreaker(new Config(name, failureThreshold, timeout, resetTimeout));
        }
    }
    ...
    public <T> Result<T, CircuitError> execute(Supplier<T> operation) {
        ...
        if (state.get() == State.OPEN) {
            return Result.err(
                    new CircuitError.CircuitOpen(
                            config.name(),
                            openedAt != null
                                    ? openedAt.plus(config.resetTimeout())
                                    : Instant.now()));
        }
    ...
    public sealed interface CircuitError
            permits CircuitError.CircuitOpen, CircuitError.Failure, CircuitError.Timeout {
    ...
    public static void register(CircuitBreaker breaker) {
        REGISTRY.put(breaker.config.name(), breaker);
    }
