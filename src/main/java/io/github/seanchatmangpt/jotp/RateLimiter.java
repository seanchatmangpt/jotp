 ...
 *
 * <p>Rate limiting is a critical resilience pattern for * that prevents service degradation by
 * controlling the rate of incoming requests. Different algorithms provide different trade-offs
 * between fairness, memory usage, and precision.
 *
 * <p>Features:
 *
 * <ul>
 *   <li><b>Token Bucket</b> — Smooth traffic flow with burst tolerance
 *   <li><b>Sliding Window</b> — Precise request counting over a time window
 *   <li><b>Per-Client</b> — Rate limiting per client identifier
 *   <li><b>Per-Second/Per-Minute</b> — Convenient factories for common time windows
 * </ul>
 *
 * <p><b>Usage:</b>
 *
...
public interface RateLimiter {
    ...
    static RateLimiter perSecond(int rate) {
        return tokenBucket(rate, rate);
    }

    static RateLimiter perMinute(int rate) {
        return tokenBucket(rate, rate / 60);
    }

    static RateLimiter tokenBucket(long capacity, double refillRate) {
        return new TokenBucket(capacity, refillRate);
    }

    static RateLimiter slidingWindow(int maxRequests, Duration window) {
    ...
    final class PerClient<K> implements RateLimiter {
        ...
        public boolean tryAcquire(K clientKey) {
        ...
        @Override
        public double currentRate() {
            return limiters.values().stream()
                    .mapToDouble(RateLimiter::currentRate)
                    .average()
                    .orElse(0);
        }
        ...
        @Override
        public long availablePermits() {
            return limiters.values().stream().mapToLong(RateLimiter::availablePermits).sum();
        }
        ...
        public void reset(K clientKey) {
            limiters.remove(clientKey);
        }
    }
}
