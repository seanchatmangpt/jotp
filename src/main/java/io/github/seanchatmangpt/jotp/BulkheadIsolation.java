 ...
 * overloaded (queue depth or process count exceeded), messages are rejected with a structured error,
 * allowing the caller to handle degradation gracefully.
 *
 * <p><strong>Architecture:</strong>
 *
 * <ul>
 *   <li><strong>Per-Feature Supervisor:</strong> Each feature (identified by {@code <F>}) has its own
 *       {@link Supervisor} with ONE_FOR_ONE restart strategy. Crashes in one feature don't affect
 *       others.
 ...
 *       the naming scheme {@code bulkhead:<F>}, enabling other processes to discover and monitor them.
 *   <li><strong>Status Tracking:</strong> Each bulkhead maintains a sealed status interface (Active,
 *       Degraded, Failed) with metrics (queue depth, process count, rejections) for observability.
 * </ul>
 *
 ...
 *   <li><strong>Pattern Matching:</strong> Switch expressions over sealed types for status handling.
...
public final class BulkheadIsolation<F, M> {
   ...
   *
   * <p>Active: all workers healthy, queues below threshold.
   * Degraded: at least one worker's queue exceeds {@code maxQueueDepth}.
   * Failed: supervisor has terminated due to max restarts exceeded.
   */
  public sealed interface BulkheadStatus permits BulkheadStatus.Active, BulkheadStatus.Degraded,
      BulkheadStatus.Failed {
  ...
  private BulkheadIsolation(
    ...
    this.supervisor =
        Supervisor.create(
            "bulkhead-" + featureId, Supervisor.Strategy.ONE_FOR_ONE, 5, Duration.ofSeconds(30));
   ...
   * selected (round-robin or least-queue), and the message is sent. If no workers exist yet, one is
   * spawned.
   *
   ...
   *
   * <p>Samples all worker queues to determine if any exceed {@code maxQueueDepth}. Returns ACTIVE,
   * DEGRADED, or FAILED.
   *
