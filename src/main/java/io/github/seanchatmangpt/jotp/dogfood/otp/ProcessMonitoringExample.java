...
public final class ProcessMonitoringExample {
  ...
  static WorkerState workerBehavior(WorkerState state, WorkerMessage msg) {
    return switch (msg) {
      case WorkerMessage.Increment() -> new WorkerState(state.messageCount() + 1, state.name());
  ...
  static DashboardState dashboardBehavior(DashboardState state, DashboardMessage msg) {
    return switch (msg) {
      case DashboardMessage.MonitorWorker(var worker, var name) -> {
        // Install a monitor on the worker
        ProcMonitor.monitor(worker);
        var newWorkers = new HashMap<>(state.workers());
        newWorkers.put(worker, name);
        yield new DashboardState(newWorkers, state.downNotifications(), state.statsCollected());
      }
  ...
  sealed interface DashboardMessage {
    record MonitorWorker(ProcRef<WorkerState, WorkerMessage> worker, String name)
  ...
  public static void main(String[] args) throws Exception {
    ...
    var worker1 = Proc.spawn(new WorkerState(0, "Worker-1"), ProcessMonitoringExample::workerBehavior);
    var worker2 = Proc.spawn(new WorkerState(0, "Worker-2"), ProcessMonitoringExample::workerBehavior);
    System.out.println("   - Worker-1 spawned");
    System.out.println("   - Worker-2 spawned\n");

    // 2. Spawn dashboard
    System.out.println("2. Creating dashboard process...");
    var dashboard = Proc.spawn(
    ...
    dashboard.send(new DashboardMessage.MonitorWorker(worker1, "Worker-1"));
    dashboard.send(new DashboardMessage.MonitorWorker(worker2, "Worker-2"));
    ...
    dashboard.send(new DashboardMessage.CollectStats());
    Thread.sleep(100);

    // Get dashboard state to see collected stats
    var dashboardState = dashboard.ask(msg -> new DashboardMessage.CollectStats(), Duration.ofSeconds(1));
    ...
    System.out.println("  - ProcMonitor.monitor() installed without affecting worker performance");
