 ...
 *
 * <p>Scenario: A distributed order notification system receives messages with time-sensitive data
 * (e.g., "order cancellation within 5 minutes of placement"). Messages that arrive late should not
 * be processed as they represent stale state. MessageExpiration ensures cleanup of expired
 * notifications.
 */
public final class MessageExpirationExample {
    ...
    public static void main(String[] args) throws InterruptedException {
        ...
        var incomingNotifications =
                new ArrayList<MessageExpiration.ExpiringMessage<NotificationMsg>>();
        ...
        for (var msg : incomingNotifications) {
            ...
            System.out.println(
                    String.format(
                            "  %s %s (age: %dms, ttl: %dms)",
                            status,
                            ((NotificationMsg.OrderNotification) msg.message()).orderId(),
                            age,
                            ttl));
        }
        ...
        System.out.println(
                "Time until expiration: " + expiration.timeUntilExpiration(expiring) + "ms");
