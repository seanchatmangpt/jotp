...
public final class StateMachineExample {
  ...
  sealed interface TurnstileEvent permits TurnstileEvent.Coin, TurnstileEvent.Pass {
    ...
    record Coin(int amount) implements TurnstileEvent {
      public Coin {
        if (amount <= 0) {
          throw new IllegalArgumentException("coin amount must be positive, got " + amount);
        }
      }
  ...
  record TurnstileData(int balance, int totalCoins) {
    public TurnstileData {
      ...
      if (totalCoins < 0) {
        throw new IllegalArgumentException("totalCoins cannot be negative, got " + totalCoins);
      }
    }
  ...
  public static void main(String[] args) throws InterruptedException {
    ...
    Thread.sleep(100);
    System.out.println("After deposits: state=" + turnstile.state() + ", data=" + turnstile.data());
    System.out.println();
    ...
    System.out.println("Sent: Coin(25¢)");
    Thread.sleep(100);
    System.out.println("After deposit: state=" + turnstile.state() + ", data=" + turnstile.data());
    System.out.println();
    ...
    System.out.println("Transitioned to Unlocked? " + (turnstile.state() instanceof TurnstileState.Unlocked));
    System.out.println();
    ...
    System.out.println("Transitioned to Locked? " + (turnstile.state() instanceof TurnstileState.Locked));
    System.out.println();
