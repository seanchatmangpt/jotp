...
public class JvmCrashRecoveryPatterns {
  ...
  public sealed interface ChargeMessage {
    /**
     * Charge request carrying an idempotency key. Sending the same key twice is safe — the second
   ...
   *
   * <p>On a {@link ChargeMessage.ChargeRequest}: if the key was already processed, return existing
   * state unchanged (deduplication). Otherwise, generate a transaction ID and record it.
   *
    ...
    try {
      return CrashRecovery.retry(
          ...
          () -> {
            ...
            // replayed — the receiver deduplicates, so the charge is applied exactly once.
            ChargeState state = chargeService.ask(req, timeout).join();
            ChargeReceipt receipt = state.processed().get(idempotencyKey);
            if (receipt == null) {
              throw new IllegalStateException("charge not recorded for key: " + idempotencyKey);
            }
            return receipt;
          });
    } finally {
  ...
  public static Result<Integer, Exception> statelessDouble(int value) {
    ...
    try {
      return CrashRecovery.retry(
          3,
          () -> {
            WorkerState result = worker.ask(new ComputeMessage.DoubleIt(value), timeout).join();
  ...
  public record CheckpointState(
    ...
    /**
     * Persist current position. In production this would write to a database or distributed log;
     * here we simply capture the current counters.
     */
    CheckpointState checkpoint() {
      return new CheckpointState(processedCount, lastProcessedKey, processedCount, lastProcessedKey);
    }
  }
   ...
   * so the process durably records its progress. On JVM crash + restart, only items after the last
   * checkpoint need to be replayed.
   *
