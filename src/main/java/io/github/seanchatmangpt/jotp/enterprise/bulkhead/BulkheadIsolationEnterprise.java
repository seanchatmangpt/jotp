 ...
 *
 * Prevents one feature from starving others by enforcing per-feature resource limits
 * (concurrent requests, queue size, memory, CPU). Uses Process-based strategy with
...
public class BulkheadIsolationEnterprise {
  ...
  private BulkheadIsolationEnterprise(BulkheadConfig config, Semaphore semaphore, ProcRef<BulkheadState, BulkheadMsg> coordinator) {
    this.config = config;
  ...
  public <T> Result<T> execute(BulkheadTask<T> task) {
    ...
    try {
      ...
      // Notify completion
      coordinator.tell(new BulkheadMsg.RequestCompleted(requestId, duration, calculateUtilization()));

      return Result.success(result);
    } catch (Exception e) {
      return Result.failure(new BulkheadException("Task execution failed: " + e.getMessage(), e));
    } finally {
      semaphore.release();
    }
  }
  ...
  private static ProcRef<BulkheadState, BulkheadMsg> spawnCoordinator(BulkheadConfig config) {
    var proc = new Proc<>(
        ...
        (BulkheadState state, BulkheadMsg msg) -> {
          return switch (msg) {
            case BulkheadMsg.RequestCompleted(var id, var duration, var utilization) ->
                handleRequestCompleted(state, duration, utilization, config);
            case BulkheadMsg.RequestRejected(var id, var reason, var utilization) ->
   ...
   */
  sealed interface BulkheadMsg permits
      BulkheadMsg.RequestCompleted,
