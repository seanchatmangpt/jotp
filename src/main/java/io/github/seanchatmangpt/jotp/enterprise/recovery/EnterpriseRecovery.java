 ...
 *
 * Implements advanced recovery strategies including circuit breaker integration,
 * fresh virtual thread per attempt (via CrashRecovery pattern), and adaptive backoff.
 *
 * State Machine:
 * - INITIAL: First attempt
 * - BACKOFF: Waiting between retries
 * - RETRY: Executing attempt N
 * - COMPLETED: Success
 * - FAILED: All attempts exhausted
 * - CIRCUIT_BREAKER_TRIPPED: Too many consecutive failures
 */
public class EnterpriseRecovery {
  ...
  private EnterpriseRecovery(RecoveryConfig config, ProcRef<RecoveryState, RecoveryMsg> coordinator) {
    this.config = config;
  ...
  private <T> Result<T> executeWithRetry(RecoveryTask<T> task, int attemptNumber) {
    ...
    try {
      ...
      long duration = System.currentTimeMillis() - startTime;
      coordinator.tell(new RecoveryMsg.AttemptFailed(attemptNumber, e.getMessage(), duration));
  ...
  private static ProcRef<RecoveryState, RecoveryMsg> spawnCoordinator(RecoveryConfig config) {
    ...
    var proc = new Proc<>(
        initialState,
        (RecoveryState state, RecoveryMsg msg) -> switch (msg) {
          case RecoveryMsg.AttemptSucceeded(var attempt, var duration) ->
              handleAttemptSucceeded(state, attempt, duration);
          case RecoveryMsg.AttemptFailed(var attempt, var error, var duration) ->
   ...
   */
  sealed interface RecoveryMsg permits
      RecoveryMsg.AttemptSucceeded,
      RecoveryMsg.AttemptFailed,
      RecoveryMsg.Shutdown {

    record AttemptSucceeded(int attemptNumber, long durationMs) implements RecoveryMsg {}

    record AttemptFailed(int attemptNumber, String error, long durationMs) implements RecoveryMsg {}
