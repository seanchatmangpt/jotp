...
public class Backpressure {
    ...
    private final CopyOnWriteArrayList<BackpressureListener> listeners =
            new CopyOnWriteArrayList<>();

    private Backpressure(
            BackpressureConfig config, ProcRef<BackpressureState, BackpressureMsg> coordinator) {
        this.config = config;
    ...
    public <T> Result<T> execute(BackpressureTask<T> task, Duration defaultTimeout) {
        ...
        try {
            ...
            return Result.failure(
                    new BackpressureException("Request failed: " + e.getMessage(), e));
        }
    }
    ...
    private static ProcRef<BackpressureState, BackpressureMsg> spawnCoordinator(
            BackpressureConfig config) {
        var initialState =
                new BackpressureState(
                        config.serviceName(),
        ...
        var handler =
                (java.util.function.BiFunction<
                                BackpressureState, BackpressureMsg, BackpressureState>)
                        (state, msg) -> {
                            return switch (msg) {
                                case BackpressureMsg.RequestCompleted(var id, var duration) ->
                                        handleRequestCompleted(state, duration, config);
                                case BackpressureMsg.RequestTimedOut(var id) ->
                                        handleRequestTimedOut(state, config);
    ...
    sealed interface BackpressureMsg
            permits BackpressureMsg.RequestCompleted,
