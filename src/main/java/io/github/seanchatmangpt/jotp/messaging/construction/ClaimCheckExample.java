 ...
 * Instead of sending the 50MB PDF over the network, we store it locally, send a small token, and
 * the worker retrieves it on-demand. This saves bandwidth and enables deferred loading.
 *
 * <p>Process flow:
 *
 * <ol>
...
public class ClaimCheckExample {
     ...
     */
    public static Result<ProcessingResult, String> processPdf(ClaimCheck.CheckedMessage checked) {
     ...
     *
     * <p>If the claim was already consumed by another worker, this returns false. Otherwise, the
     * claim can be reprocessed idempotently.
     *
     * @param checked the CheckedMessage token
    ...
    private static String simulateOcrExtraction(PdfDocument doc) {
        ...
        return "Extracted text: " + doc.filename() + " contains " + doc.content.length + " bytes";
    }
