 ...
 *
 * <ul>
 *   <li>Named process steps → ProcessRegistry (OTP: register/2)
...
public final class ProcessManager<C> {
    ...
    public record Step<C>(
            String name,
            BiFunction<C, Object, Object> handler,
            BiFunction<C, Object, Void> rollbackHandler) {}
    ...
    sealed interface WorkflowEvent
            permits WorkflowEvent.StartWorkflow,
                    WorkflowEvent.StepCompleted,
                    WorkflowEvent.StepFailed,
                    WorkflowEvent.RollbackStarted,
                    WorkflowEvent.WorkflowDone {
     ...
     *
     * @param managerId unique identifier for this manager (used in ProcessRegistry)
     */
        ...
        StateMachine.TransitionFn<WorkflowState, WorkflowEvent, WorkflowData> transitionFn =
                (state, event, data) -> {
                    return switch (state) {
                        ...
                        case WorkflowState.Failed _, WorkflowState.Complete _ ->
                ...
                () -> {
                    ...
                    if (workflowData.context.getLastError().isPresent()) {
                        return Result.failure(
                                "Workflow failed: "
                                        + workflowData.context.getLastError().get().getMessage());
                    }
    ...
    private void processNextStep(int stepIndex) {
        ...
        try {
            Object output = step.handler.apply(workflowData.context, workflowData.currentInput);
    ...
    private void rollback(int fromIndex) {
        ...
        for (int i = fromIndex; i >= 0; i--) {
            ...
            try {
                if (output != null) {
                    step.rollbackHandler.apply(workflowData.context, output);
                }
            } catch (Exception e) {
