...
/**
 * Base sealed interface for all messaging patterns. All messages in the JOTP messaging framework
 * inherit from this type, enabling pattern matching and type-safe routing.
 */
public sealed interface Message extends Serializable
    ...
    /**
     * Command message: request with expected reply. Used for synchronous-style RPC over async
     * channels (Vernon: Command Message).
     */
    record CommandMsg(
            UUID messageId,
            long createdAt,
    ...
    /**
     * Event message: immutable notification, usually 1:N broadcast. Used for publish-subscribe
     * patterns (Vernon: Event Message, Document Message).
     */
    record EventMsg(UUID messageId, long createdAt, String eventType, Object payload)
            implements Message {
        public EventMsg {
    ...
    /**
     * Query message: request for information. Used for dynamic routing and process registry
     * lookups.
     */
    record QueryMsg(UUID messageId, long createdAt, String queryType, Object criteria)
            implements Message {
        public QueryMsg {
    ...
    /**
     * Document message: entire domain entity wrapped as a message. Used when transferring large or
     * complex data structures (Vernon: Document Message).
     */
    record DocumentMsg(
            UUID messageId,
            long createdAt,
    ...
    static CommandMsg command(String commandType, Object payload, Object replyTo) {
        return new CommandMsg(
                UUID.randomUUID(), System.currentTimeMillis(), commandType, payload, replyTo);
    }
    ...
    static DocumentMsg document(String documentType, byte[] documentBytes) {
        return new DocumentMsg(
                UUID.randomUUID(), System.currentTimeMillis(), documentType, documentBytes);
    }
}
