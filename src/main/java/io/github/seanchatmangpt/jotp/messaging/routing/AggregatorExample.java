 ...
 * order) and combining them back into a complete order. Shows timeout handling and out-of-order
 * part assembly.
 */
public final class AggregatorExample {
    ...
    public record CompleteOrder(String orderId, List<OrderItemMessage> items) {
        ...
        public String toString() {
            return String.format("Order[%s with %d items]", orderId, items.size());
        }
    }
    ...
    public static void main(String[] args) throws Exception {
        ...
        var aggregator =
                Aggregator.<OrderItemMessage, CompleteOrder>create(
                        5,
        ...
        for (int seqNum : partOrder) {
            ...
            Optional<CompleteOrder> result = aggregator.aggregate(part);
    ...
    private static List<Splitter.MessagePart<OrderItemMessage>> createOrderItems(
            String orderId, UUID correlationId, List<OrderItemMessage> items) {
        return Splitter.split(items.get(0), _ -> items, correlationId);
    }
}
