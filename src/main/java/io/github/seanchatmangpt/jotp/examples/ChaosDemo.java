  ...
  static class Metrics {
    ...
    public void printMetrics() {
      ...
      if (!recoveryTimes.isEmpty()) {
        ...
        long p999 =
            sorted.size() > 1000 ? sorted.get((int) (sorted.size() * 0.999)) : sorted.getLast();
          ...
          (processed, msg) ->
              switch (msg) {
                case WorkerMessage.ProcessRequest(var id, var data) -> {
                  // Simulate processing
                  Thread.sleep(5);
                  yield new Proc.StateResult<>(processed + 1, new WorkerResponse.Success(id,
              ...
              if (System.currentTimeMillis() - startTime < DEMO_DURATION_MS) {
                ...
                try {
                  ...
                  System.out.printf("[%ds] Chaos: Killing worker-%d%n",
                      (System.currentTimeMillis() - startTime) / 1000, randomWorker);
                } catch (Exception e) {
        ...
        scheduler.scheduleAtFixedRate(
            () -> {
              if (System.currentTimeMillis() - startTime < DEMO_DURATION_MS) {
                ...
                try {
                  worker.tell(new WorkerMessage.ProcessRequest(randomWorker,
        ...
        scheduler.scheduleAtFixedRate(
            () -> {
              if (System.currentTimeMillis() - startTime < DEMO_DURATION_MS) {
                ...
                System.out.printf("[%ds] Stats: %,d req/sec, deaths: %d%n", elapsedSec, rps,
                    metrics.totalDeaths.get());
              }
            },
            STATS_INTERVAL_MS,
            STATS_INTERVAL_MS,
