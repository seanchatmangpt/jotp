...
public class ChaosDemo {
    ...
    static class Worker {
        static Proc<Integer, WorkerMessage> spawn(int workerId) {
            return Proc.spawn(
    ...
    static class Coordinator {
        static Proc<Void, WorkerMessage> spawn(
                List<ProcRef<Integer, WorkerMessage>> workers,
                Metrics metrics,
                Duration duration,
                long chaosKillIntervalMs) {
            return Proc.spawn(null, (state, msg) -> state);
        }
    }
    ...
    public static void main(String[] args) throws Exception {
        ...
        // Add worker children - store as ProcRef<Integer, Object> for flexibility
        List<ProcRef<Integer, Object>> workers = new ArrayList<>();
                    ...
                    supervisor.supervise(
                            ...
                            (state, msg) ->
                                    switch (msg) {
                                        ...
                                        case WorkerMessage.GetStatus() -> state;
                                        default -> state;
                                    });
            workers.add(workerRef);
        }

        // Supervisor is already started after create()
        ...
        var loadTask =
                scheduler.scheduleAtFixedRate(
                        () -> {
                            if (System.currentTimeMillis() - startTime < DEMO_DURATION_MS) {
                                int randomWorker = ThreadLocalRandom.current().nextInt(NUM_WORKERS);
                                ProcRef<Integer, Object> worker = workers.get(randomWorker);
                                try {
                                    worker.tell(
