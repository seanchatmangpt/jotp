...
public class EcommerceOrderService {
    ...
    public Result<SagaOutcome, SagaError> executeOrderCreation(OrderData orderData) {
      ...
      if (paymentResult.isFailure()) {
        ...
        return Result.failure(
            new SagaError("Payment failed: " + paymentResult.failureReason(), List.of()));
      }
      ...
      var inventoryResult =
          inventoryService.reserve(orderData.items().get(0).productId(), 1);
      if (inventoryResult.isFailure()) {
        ...
        return Result.failure(
            new SagaError(
                "Inventory failed",
                List.of("Refunded payment: " + transactionId)));
      }
      ...
      return Result.success(
          new SagaOutcome(
              ...
              List.of(
                  ...
                  new CompensationRecord("Shipping", trackingNumber, "SUCCEEDED"))));
    }
  ...
  public static void main(String[] args) {
    System.out.println("╔═══════════════════════════════════════════════════════════════════════════╗");
    System.out.println("║  E-Commerce Order Service: JOTP Pattern Integration Example              ║");
    System.out.println("╚═══════════════════════════════════════════════════════════════════════════╝\n");
    ...
    var coordinator = new OrderSagaCoordinator(paymentService, inventoryService, shippingService, auditLog, Duration.ofSeconds(5));
    ...
    if (result.isSuccess()) {
      ...
      for (var step : outcome.steps()) {
        System.out.println("     - " + step.service() + ": " + step.id() + " [" + step.status() + "]");
      }
    } else {
  ...
  public static class InMemoryAuditBackend implements EventSourcingAuditLog.AuditBackend<OrderState> {
