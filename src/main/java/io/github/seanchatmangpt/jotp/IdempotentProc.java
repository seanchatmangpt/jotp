 ...
 *
 * <p>The dedup window is a bounded FIFO/LRU cache backed by a {@link LinkedHashMap} with
 * {@code removeEldestEntry} eviction. The cache is wrapped with {@link
...
public final class IdempotentProc<S, M> {
  ...
  public interface Idempotent {
    /**
     * Returns the idempotency key for this message. Two messages with the same key are considered
     * duplicates; the second and subsequent deliveries within the dedup window are dropped.
     *
  ...
  private IdempotentProc(Proc<S, M> delegate, int dedupCacheSize) {
    ...
    // Bounded LRU cache: access-order LinkedHashMap evicts eldest when size exceeds dedupCacheSize.
  ...
  public static <S, M> IdempotentProc<S, M> wrap(Proc<S, M> proc, int dedupCacheSize) {
    ...
    if (dedupCacheSize <= 0)
      throw new IllegalArgumentException("dedupCacheSize must be positive, got: " + dedupCacheSize);
   ...
   *
   * <p>If {@code message} implements {@link Idempotent} and its {@link Idempotent#idempotencyKey()}
   * was already seen within the current dedup window, the message is silently dropped. Otherwise it
   * is forwarded to the delegate via {@link Proc#tell}.
   *
  ...
  public void tell(M message) {
    if (message instanceof Idempotent idempotent) {
      ...
      // putIfAbsent under the synchronized map lock; returns null iff key was absent (first time)
