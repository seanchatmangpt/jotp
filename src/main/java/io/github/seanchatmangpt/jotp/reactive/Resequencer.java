...
/**
 * Resequencer — EIP routing pattern that collects out-of-sequence messages and re-emits them in the
 * correct order to the downstream channel.
 *
 * <p>When messages arrive out of order (due to parallel processing, network reordering, or producer
 * batching), the Resequencer buffers them until the expected next message arrives, then flushes the
 * longest contiguous in-order run to downstream.
 *
 ...
 *
 * <ol>
 *   <li>Insert the new entry into the sorted pending map
 *   <li>Flush the longest contiguous run starting at {@code nextExpected}
 *   <li>Return the updated state
 * </ol>
 *
 * <p>OTP analogy: a {@code gen_server} maintaining a {@code gb_trees} (sorted map) of out-of-order
 * messages, flushing contiguous sequences when gaps close — pure state machine with no mutable
 * shared data.
 *
...
public final class Resequencer<T, K extends Comparable<K>> implements AutoCloseable {
    ...
    record State<T, K>(K nextExpected, TreeMap<K, T> pending, long resequenced) {
        ...
        static <T, K extends Comparable<K>> State<T, K> initial() {
            // Cast needed: Comparator.naturalOrder() infers Comparator<Object>; safe for K extends
            // Comparable<K>
     ...
     * @param nextKey given key K, returns the key of the next expected message (e.g. {@code n -> n
     *     + 1} for {@code Long} keys)
     * @param downstream channel that receives messages in sequence order
     */
    public Resequencer(
            Function<T, K> keyExtractor, Function<K, K> nextKey, MessageChannel<T> downstream) {
        ...
        this.proc =
                new Proc<>(
                        State.initial(),
                        (State<T, K> state, Entry<T, K> entry) -> {
                            // Pure state transition: insert → flush contiguous run → return new
                            // state
    ...
    /**
     * Submits a message to the resequencer. May be called from any thread. The message is enqueued
     * in the backing Proc's mailbox and forwarded downstream when its turn arrives in sequence
     * order.
     */
    ...
    /**
     * Returns the count of messages forwarded downstream in correct sequence order. Reads from JOTP
     * process statistics — consistent with OTP {@code sys:statistics/1}.
     */
    ...
    /**
     * Returns the backing {@link Proc} for JOTP introspection via {@link ProcSys}. Use {@link
     * ProcSys#statistics(Proc)}, {@link ProcSys#getState(Proc)}, or {@link
     * ProcSys#suspend(Proc)}/{@link ProcSys#resume(Proc)} for runtime control.
     */
